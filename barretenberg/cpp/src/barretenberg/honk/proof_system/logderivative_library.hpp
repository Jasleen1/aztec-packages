...
//                        for (size_t i = 0; i < circuit_size; ++i) {
//                            // TODO(https://github.com/AztecProtocol/barretenberg/issues/940): avoid get_row if
//                            possible. typename Flavor::AllValues row; AVM_TRACK_TIME(key_getrow, (row =
//                            polynomials.get_row(i))); bool has_inverse = false; AVM_TRACK_TIME(key_exists,
//                            (has_inverse = Relation::operation_exists_at_row(row))); if (!has_inverse) {
//                                continue;
//                            }
//                            FF denominator = 1;
...
//                                               bb::constexpr_for<0, WRITE_TERMS, 1>([&]<size_t write_index> {
//                                                   auto denominator_term =
//                                                       Relation::template compute_write_term<Accumulator,
//                                                       write_index>(
//                                                           row, relation_parameters);
//                                                   denominator *= denominator_term;
//                                               });
