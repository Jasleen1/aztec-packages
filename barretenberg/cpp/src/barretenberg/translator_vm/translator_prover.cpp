...
void TranslatorProver::compute_witness(CircuitBuilder& circuit_builder)
{
    ...
    auto wires_and_polys = zip_view(key->polynomials.get_wires(), circuit_builder.wires);
    for (auto&& tuple : wires_and_polys) {
        auto& wire_poly = std::get<0>(tuple);
        auto& wire = std::get<1>(tuple);

        run_loop_in_parallel(circuit_builder.num_gates, [&circuit_builder, &wire_poly, &wire](size_t start, size_t end) {
            for (size_t i = start; i < end; i++) {
                wire_poly[i] = circuit_builder.get_variable(wire[i]);
            }
        });
    ...
    // auto wires_and_polys = zip_view(key->polynomials.get_wires(), circuit_builder.wires);
    // for (size_t idx = 0; idx < wires_and_polys.size(); ++idx) {
    //     auto& wire_poly = std::get<0>(wires_and_polys[idx]);
    //     auto& wire = std::get<1>(wires_and_polys[idx]);
